...
class Mainwp_WPvivid_Extension_Subpage
{
        ...
        if (isset($_GET['id']) && !empty($_GET['id'])) {
            ...
            <script>
                ...
                function mwp_wpvivid_click_remote_page() {
                    ...
                    ?>
                    var location = "admin.php?page=SiteOpen&newWindow=yes&websiteid=<?php esc_html_e($_GET['id']); ?>&location=<?php esc_html_e(base64_encode($location)); ?>";
    ...
    static function renderSubBackuppage($check_pro)
    {
        ...
        $backup_custom_setting=Mainwp_WPvivid_Extension_DB_Option::get_instance()->wpvivid_get_option(sanitize_text_field($_GET['id']), 'backup_custom_setting', array());
    ...
    static public function output_database_table($base_tables, $other_tables){
        ...
        foreach ($base_tables as $base_table){
            ...
            $has_base_table = true;
            $base_table_html .= '<div class="mwp-wpvivid-custom-database-table-column">
                                                <label style="width:100%;overflow: hidden;text-overflow: ellipsis;white-space: nowrap; padding-top: 3px;" 
                                                title="'.esc_html($base_table["table_name"]).'|Rows:'.$base_table["table_row"].'|Size:'.$base_table["table_size"].'">
                                                    ...
                                                    '.esc_html($base_table["table_name"]).'|Rows:'.$base_table["table_row"].'|Size:'.$base_table["table_size"].'
                                                </label>
                                            </div>';
        ...
        foreach ($other_tables as $other_table){
            ...
            $other_table_html .= '<div class="mwp-wpvivid-custom-database-table-column">
                                                <label style="width:100%;overflow: hidden;text-overflow: ellipsis;white-space: nowrap; padding-top: 3px;" 
                                                title="'.esc_html($other_table["table_name"]).'|Rows:'.$other_table["table_row"].'|Size:'.$other_table["table_size"].'">
                                                    ...
                                                    '.esc_html($other_table["table_name"]).'|Rows:'.$other_table["table_row"].'|Size:'.$other_table["table_size"].'
                                                </label>
                                            </div>';
        ...
        if($has_base_table) {
            $base_tables_html .= '<div class="mwp-wpvivid-custom-database-wp-table-header" style="border:1px solid #e5e5e5;">
                                        <div>
                                            <div style="float: left; margin-right: 10px;">
                                                <label><input type="checkbox" class="mwp-wpvivid-database-table-check mwp-wpvivid-base-table-all-check" ' . esc_attr($base_table_all_check) . ' />WordPress Tables</label>
                                            </div>
                                            <small>
                                                <div class="mwp-wpvivid-tooltip" style="float: left; margin-top: 6px; line-height: 100%; white-space: normal;">?
                                                    <div class="mwp-wpvivid-tooltiptext">The tables are created by WordPress. Select all unless you are a WordPress specialist.</div>
                                                </div>
                                            </small>
                                            <div style="clear: both;"></div>
                                        </div>
        ...
        if($has_other_table) {
            $other_tables_html .= '<div class="mwp-wpvivid-custom-database-other-table-header" style="border:1px solid #e5e5e5;">
                                        <div>
                                            <div style="float: left; margin-right: 10px;">
                                                <label><input type="checkbox" class="mwp-wpvivid-database-table-check mwp-wpvivid-other-table-all-check" ' . esc_attr($other_table_all_check) . ' />Other Tables</label>
                                            </div>
                                            <small>
                                                <div class="mwp-wpvivid-tooltip" style="float: left; margin-top: 6px; line-height: 100%; white-space: normal;">?
                                                    <div class="mwp-wpvivid-tooltiptext">Other tables are created by your plugins or themes, please select with caution.</div>
                                                </div>
                                            </small>
                                            <div style="clear: both;"></div>
                                        </div>
        ...
        $database_table_html = $base_tables_html . $div . $other_tables_html;
    ...
    static public function output_additional_database_table($database_array){
        ...
        foreach ($database_array as $database){
            $database_html .= '<div class="mwp-wpvivid-custom-database-table-column">
                                                   <label style="width:100%;overflow: hidden;text-overflow: ellipsis;white-space: nowrap; padding-top: 3px;" title="'.esc_html($database).'">
                                                       <input type="checkbox" option="additional_db" name="'.$database.'" value="'.esc_html($database).'" checked />
                                                       '.esc_html($database).'
                                                   </label>
                                               </div>';
        }
        $database_html .= '<div style="clear: both;"></div>';
        $database_html .= '<div style="margin-top: 10px;">
                                                <input class="ui green mini button mwp-wpvivid-additional-db-add" type="button" value="Select and Add" />
                                                <input class="ui green mini button mwp-wpvivid-additional-db-table-close" type="button" value="Close" />
                                           </div>';
